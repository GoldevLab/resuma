    ...
    if let Ok(s) = status {
        if s.success() {
            ...
            for (name, from) in assets {
                let to = Path::new("crates/resuma-server/assets").join(name);
                if from.exists() {
...
fn routes_command(path: &Path, generate: bool) -> Result<()> {
    ...
    for r in &routes {
        let layouts = if r.is_layout {
            ...
            resuma_router::layout_chain_for(&r.pattern, &layouts_index)
        };
        println!(
            "  {:<32} → {} ({}{}) layouts={:?}",
            r.pattern,
...
fn generate_pages_mod(routes: &[resuma_router::DiscoveredRoute]) -> String {
...
fn module_rust_path(route: &resuma_router::DiscoveredRoute) -> String {
    let stem = route
        .file
        .file_stem()
        .and_then(|s| s.to_str())
...
fn generate_pages_registry(routes: &[resuma_router::DiscoveredRoute]) -> String {
