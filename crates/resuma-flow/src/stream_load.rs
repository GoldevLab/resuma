...
use resuma_core::stream_chunk;
use resuma_ssr::{
    render_body_and_payload, stream_head, stream_tail, PageOptions, StreamChunk,
};
use serde_json::Value;

use crate::cache::loader_cache;
use crate::registry::dispatch_load;
use crate::runtime::{take_deferred_stream_plan, DeferredStreamPlan};
...
fn render_chunk_view(name: &str, value: &Value) -> View {
    ...
    View::Element(resuma_core::view::Element {
        tag: "pre".into(),
        attrs: vec![resuma_core::view::Attr {
            name: "class".into(),
            value: resuma_core::view::AttrValue::Static("resuma-stream-loaded".into()),
        }],
        children: vec![resuma_core::view::Child::Text(
        ...
        for name in plan.deferred {
            match dispatch_load(&name, req.clone()).await {
                Ok(value) => {
                    ...
                    let inner = resuma_ssr::render_view(&render_chunk_view(&name, &value));
                    let chunk = resuma_ssr::render_view(&stream_chunk(&name, inner));
                    yield Ok(chunk);
                }
                Err(err) => {
                    let inner = format!(
...
pub fn install_deferred_stream_hook() {
    resuma_server::set_deferred_stream_hook(deferred_stream_hook);
}
