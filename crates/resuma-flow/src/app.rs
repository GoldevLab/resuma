...
use resuma_server::ResumaApp;

use crate::errors::{error_page, FlowError};
use crate::cache::{loader_cache, merge_cache_control};
use crate::layout::apply_layouts;
use crate::match_route::match_route;
use crate::middleware::run_middleware;
use crate::pages::{discover_pages, FlowPageRegistry};
use crate::request::FlowRequest;
use crate::routes::attach_flow_routes;
use crate::runtime::{first_load_error, stage_deferred_stream_plan, with_request_deferred};
...
fn render_with_flow(mut req: FlowRequest, entry: PageEntry, deferred_streaming: bool) -> View {
    ...
    if deferred_streaming && !deferred.is_empty() {
        ...
        if let Some(cache) = merge_cache_control(&hints) {
            resuma_server::stage_response_cache_control(cache);
        }
