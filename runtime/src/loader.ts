 ...
 *
 * Inline JS bodies (passed via `data-r-inline:<event>`) win over both:
 * they are compiled to a function once and cached.
 */

type Handler = (
  event: Event,
  state: Record<string, unknown>,
  resuma: { action(name: string, args: unknown[]): Promise<unknown>; signals: Map<string, unknown>; }
...
export async function resolveHandler(ref: string, inline: string | null): Promise<Handler> {
  ...
  const r = (window as unknown as { __resuma: { handlers: Record<string, Record<string, string>>; loaded: Map<string, Record<string, Function>>; } }).__resuma;
...
function compileInline(src: string): Handler {
  // The body might be either a full arrow function `(_)=>{...}` or a raw
  // expression. We wrap raw expressions to give them an event/state/resuma
  // signature.
