...
mod tests {
    ...
    fn req_with_user(user: &str) -> FlowRequest {
        let req = FlowRequest {
            headers: BTreeMap::from([("cookie".into(), format!("{}={user}", DEMO_USER_COOKIE))]),
            ..Default::default()
        };
        attach_session(req).expect("session")
    }

    #[test]
    fn normalize_title_rejects_empty_and_trims() {
        assert!(normalize_title("  hi  ").unwrap() == "hi");
        assert!(normalize_title("").is_err());
        assert!(normalize_title(&"x".repeat(MAX_TITLE_LEN + 1)).is_err());
    }

    #[test]
    fn assert_owner_allows_admin() {
        let alice = req_with_user("alice");
        assert!(assert_owner("bob", &alice).is_ok());
    }

    #[test]
    fn assert_owner_denies_other_users() {
        let guest = req_with_user("guest");
        assert!(matches!(
            assert_owner("alice", &guest),
            Err(ResumaError::Forbidden(_))
        ));
    }

    #[test]
    fn can_add_todo_enforces_limit() {
        assert!(can_add_todo(MAX_TODOS - 1).is_ok());
        assert!(matches!(
            can_add_todo(MAX_TODOS),
            Err(ResumaError::Forbidden(_))
        ));
    }
}
